...
export function FormattedLabel({
  ...
  if (forceEnglish) {
    return <strong className={`font-black ${enClass}`}>{en}</strong>;
  }

  if (level === 'A1' && (!en || en === ro)) {
    return <strong className={`font-black ${enClass}`}>{ro}</strong>;
  }

  return (
    <div className="flex flex-col text-left leading-tight">
      <strong className={`font-black ${enClass} mb-1`}>{en}</strong>
      <span className={`${roClass} font-medium text-[0.85em] italic`}>{ro}</span>
    </div>
  );
}
